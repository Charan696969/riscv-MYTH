...
\TLV
   ...
   |cpu
      @0
         $reset = *reset;
         $pc[31:0] = >>1$reset ? 32'd0 : >>3$valid_taken_br ? >>3$br_tgt_pc : >>3$valid_load ? >>3$inc_pc : >>3$valid_jump && >>3$is_jal ? >>3$br_tgt_pc : >>3$valid_jump && >>3$is_jalr ? >>3$jalr_tgt_pc : >>1$inc_pc;
         $imem_rd_en = !$reset;
      ...
      @5
         ...
         *passed = (|cpu/xreg[10]>>5$value == (1+2+3+4+5+6+7+8+9)) && (|cpu/xreg[15]>>5$value == 45);
